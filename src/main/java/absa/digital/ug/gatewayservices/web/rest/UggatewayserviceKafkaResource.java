...
import absa.digital.ug.gatewayservices.config.KafkaSseConsumer;
import absa.digital.ug.gatewayservices.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/uggatewayservice-kafka")
public class UggatewayserviceKafkaResource {

    private final Logger log = LoggerFactory.getLogger(UggatewayserviceKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public UggatewayserviceKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
